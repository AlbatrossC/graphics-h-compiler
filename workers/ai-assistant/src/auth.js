...
function getSessionToken(request) {
  ...
  return out.get(SESSION_COOKIE_NAME) || '';
}
...
export async function identifyRequest(request, env, body, options = {}) {
...
export async function identifyFromCookieOnly(request, env, options = {}) {
  const includeDebug = options.includeDebug === true;
  const sessionState = await resolveSessionIdentity(request, env);
  if (sessionState.status === 'ok') {
    return sessionState.identity;
